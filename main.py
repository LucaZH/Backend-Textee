...
async def upload(file: UploadFile = File(...),user: _schemas.User = _fastapi.Depends(_services.get_current_user),db: _orm.Session = _fastapi.Depends(_services.get_db)):
    try:
        ...
        file_name = "./files/" + file.filename.replace(" ", "-")
        with open(file_name, 'wb') as f:
            f.write(contents)

        output = utils.textee(file_name)
        db_file = models.File(url=file_name, user_id=user.id)
        db.add(db_file)
        db.commit()
        db.refresh(db_file)

        new_history = models.History(output=",".join(str(x) for x in output))
        db.add(new_history)
        db.commit()
...
@app.post("/upload_guest")
async def upload(file: UploadFile = File(...),db: _orm.Session = _fastapi.Depends(_services.get_db)):
    try:
        # print(os.getcwd())
        contents = file.file.read()
        with open("./files/"+file.filename, 'wb') as f:
            f.write(contents)
        file_name = "./files/"+file.filename.replace(" ", "-")
        # db_file = models.File(url=file_name, user_id="Guest")
        # db.add(db_file)
        # db.commit()
    except Exception:
        print(Exception)
...
def get_history(user: _schemas.User = _fastapi.Depends(_services.get_current_user), db: _orm.Session = _fastapi.Depends(_services.get_db)):
    try:
        ...
        output=db.query(models.History.output).filter_by(id=[history[0]for history in histories])
